...
const Detalle = () => {
  ...
  useEffect(() => {
    const getDetallesDb = () => {
      dispatch(actions.getPokemonDb(name));
    };
    async function getData() {
      const detalles = await statePoke.find((poke) => poke.name === name);
      if (detalles) {
        setDetalle(detalles);
      } else {
        getDetallesDb(name);
      ...
      {!detalle && !detalleDb ? (
        ...
        <>
          <div className={DetalleCss.setTipo}>
            ...
            {detalle
              ? detalle.types[1]
                ? ((tipo2 = detalle.types[1].name), (tipoImg2 = tipo2 + "Img"))
                : tipo2
              : detalleDb
              ? detalleDb.types[1]
          ...
          <div className={`${DetalleCss.card} ${DetalleTiposCss[tipo]}`}>
            <div className={DetalleCss.headerCard}>
              ...
              <p>HP: {detalle ? detalle.hp : detalleDb.hp}</p>
              <div>
                <div
                  title={tipo}
                  className={`${DetalleTiposCss.typeImg} ${DetalleTiposCss[tipoImg]}`}
                ></div>
                {tipo2 ? (
                  <div
                    title={tipo2}
                    className={`${DetalleTiposCss.typeImg} ${DetalleTiposCss[tipoImg2]}`}
                  ></div>
                ) : (
                  <></>
                )}
            ...
            <div className={`${DetalleCss.imagen} ${DetalleTiposCss[tipoDb]}`}>
              <h2 className={DetalleCss.id}>
                #{detalle ? detalle.id : detalleDb.id}
              </h2>
              <img
                src={
                  detalle
                ...
                alt={detalle ? detalle.name : detalleDb.name}
              ></img>
            </div>
            ...
            <div className={DetalleCss.stats}>
              <div className={DetalleCss.stat}>
                ...
                ></div>
                <p>Attack:</p>
                <span>{detalle ? detalle.attack : detalleDb.attack}</span>
              </div>
              <hr />
              <div className={DetalleCss.stat}>
                <div
                ...
                ></div>
                <p>Defense:</p>
                <span> {detalle ? detalle.defense : detalleDb.defense}</span>
              </div>
              <hr />
              <div className={DetalleCss.stat}>
                <div
                ...
                ></div>
                <p>Speed:</p>
                <span> {detalle ? detalle.speed : detalleDb.speed}</span>
              </div>
            </div>
          </div>
        </>
